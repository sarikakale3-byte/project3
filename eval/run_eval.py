"""Evaluation harness for the Insurance Claim Processing Assistant."""
...
def run_eval() -> dict:
    ...
    for case in EVAL_DATASET:
        ...
        context_texts = [
            c.content for c in rag_answer.contexts
        ]
    ...
    out_path = os.path.join(
        RESULTS_DIR,
        f"eval_{time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())}.json",
    )
...
def _summarize(items: list[dict]) -> dict:
    def avg(section: str, metric: str) -> float:
        values = [item[section][metric] for item in items]
...
def _print_summary(summary: dict, out_path: str):
    print("\n=== Insurance Assistant Evaluation Summary ===")
